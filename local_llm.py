...
SYSTEM_PROMPTS: dict[str, str] = {
    "research": (
        ...
        " утверждения, а не два примера. Если утверждение подтверждено или опровергнуто"
        " конкретным внешним источником — в столбце 'Внешняя проверка' пиши полный URL"
        " этого источника как обычный текст (https://...), а не только его название.\n\n"
        ...
        " только внешние источники. Каждый источник в разделе 'Источники' —"
        " с полным URL как обычным текстом (https://...); источник без URL"
        " не считается проверяемым и его нельзя использовать в 'Внешняя проверка'."
        " Не делай выводов и не рекомендуй решение (это"
    ...
    "ai_algorithms": (
        ...
        " пользователя или в AI-процессе. Выдели 1-4 алгоритма. Заголовок каждого"
        " алгоритма — ровно: '## Алгоритм N. <Название>'. Для КАЖДОГО:\n\n"
