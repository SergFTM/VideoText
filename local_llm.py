...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "ai_skills": (
        "Ты проектируешь набор AI-скиллов на основе материалов видео — для агента"
        " в Claude Code / Cursor. Скилл = переиспользуемая инструкция, которая"
        " описывает, ЧТО ИИ делает, на КАКИЕ данные смотрит и по КАКИМ правилам"
        " принимает решение (а не жёсткий код). Стиль: деловой, конкретный, без воды.\n\n"
        "Сначала определи 2-5 скиллов, которые реально вытекают из материала."
        " Для КАЖДОГО скилла дай блок по структуре:\n"
        "1. Название скилла — короткое, в повелительном наклонении (что он делает)\n"
        "2. Назначение — какую повторяемую задачу закрывает, когда вызывать\n"
        "3. Входные данные — что ИИ должен смотреть и какие данные брать"
        " (источники, поля, форматы)\n"
        "4. Сравнение — что с чем сравнивать и по каким признакам\n"
        "5. Алгоритм действий — пошаговый порядок (нумерованный список)\n"
        "6. Правила и критерии — какие правила применять, что проверять,"
        " что отбирать, пороги и условия\n"
        "7. Вывод / результат — что скилл возвращает и в каком виде; по каким"
        " признакам делается вывод\n"
        "8. Граница с детерминированным кодом — что отдаём на усмотрение ИИ,"
        " а что должно быть жёстко закодировано\n\n"
        "ЗАПРЕТЫ: не выдумывай несуществующие API/данные/пороги — если данных нет,"
        " пиши 'TBD: уточнить ...'. Не пересказывай видео. Не описывай скилл,"
        " которого материал не подразумевает. Используй markdown-заголовки и списки."
    ),
    "ai_algorithms": (
        "Ты описываешь алгоритмы действий для ИИ-агента на основе материалов видео —"
        " то, что агент выполняет по шагам, чтобы получить результат. Стиль: точный,"
        " пошаговый, без воды; формулировки в повелительном наклонении ('бери',"
        " 'сравни', 'проверь', 'отбери', 'сделай вывод').\n\n"
        "Выдели 1-4 алгоритма, которые вытекают из материала. Для КАЖДОГО:\n"
        "1. Название и цель — какую задачу решает алгоритм\n"
        "2. Предусловия — что должно быть на входе, какие данные нужны\n"
        "3. Какие данные смотреть и брать — источники, поля, частота обновления\n"
        "4. Шаги (нумерованный список) — что делать по порядку: что взять,"
        " что с чем сравнить, что вычислить, что проверить, что отобрать\n"
        "5. Правила и условия — пороги, фильтры, ветвления ('если X — то Y')\n"
        "6. Критерии вывода — по каким признакам делается финальное решение\n"
        "7. Результат — что выдаёт алгоритм и в каком формате\n"
        "8. Граничные случаи и риски — где алгоритм может сломаться, что делать тогда\n\n"
        "ЗАПРЕТЫ: не выдумывай числовые пороги, формулы и источники, которых нет"
        " в материалах — помечай '[TBD: ...]'. Не пересказывай видео по хронологии."
        " Каждый шаг — конкретное действие, а не общая фраза. Используй"
        " markdown-заголовки и нумерованные списки."
    ),
}
...
def build_expand_prompt(
    *,
    ...
    instruction = {
        ...
        "ai_skills":     "Спроектируй возможные AI-скиллы по структуре из системного промпта.",
        "ai_algorithms": "Опиши возможные алгоритмы действий для ИИ по структуре из системного промпта.",
