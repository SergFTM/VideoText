...
> ⚠️ Ветка «конвейер верификации» меняет схему Prisma: у `Expansion` появились
> версии (`version`, `fromVersion`, уникальность по `videoId+mode+version`) и поле
> `verdict`. Поэтому `prisma db push` и `prisma generate` из блока выше при этом
> обновлении **обязательны, а не опциональны** — без них сервер стартует на старом
> сгенерированном клиенте и падает на первой же генерации артефакта. Старые строки
> `Expansion` получают `version = 1` и `verdict = NULL`; чтобы вердикт по репорту
> снова блокировал ТЗ, репорт нужно перегенерировать.
